        ...
        return StageResultEnvelope(
            ...
            evidence=[
                EvidenceItem(
                    ...
                    command="ai-team runtime dry-run" if evidence_kind == "command" else "",
...
def _dry_run_artifact_content(stage: str, context: StageExecutionContext) -> str:
    ...
    if stage == "Dev":
        return (
            ...
            "## Self Verification\n"
            "- command: ai-team runtime dry-run\n"
            "- result: passed\n\n"
    ...
    return (
        ...
        "- Runtime driver preserved the AI_Team gates and waits for the human final decision.\n"
    )


def _stage_environment(request: StageExecutionRequest) -> dict[str, str]:
    return {
        "AI_TEAM_REPO_ROOT": str(request.repo_root),
        "AI_TEAM_SESSION_ID": request.session_id,
        "AI_TEAM_STAGE": request.contract.stage,
        "AI_TEAM_RUN_ID": request.run_id,
        "AI_TEAM_CONTRACT_PATH": str(request.contract_path),
        "AI_TEAM_CONTEXT_PATH": str(request.context_path),
        "AI_TEAM_RESULT_BUNDLE": str(request.result_path),
        "AI_TEAM_OUTPUT_SCHEMA": str(request.output_schema_path),
        "AI_TEAM_ARTIFACT_DIR": str(request.state_store.load_session(request.session_id).artifact_dir),
    }
...
def _build_codex_prompt(request: StageExecutionRequest) -> str:
    ...
    artifact_name = artifact_name_for_stage(request.contract.stage)
    return (
        f"You are the {request.contract.stage} stage worker inside the AI_Team runtime driver.\n"
        "Execute exactly this stage. Do not call ai-team commands and do not advance workflow state.\n"
