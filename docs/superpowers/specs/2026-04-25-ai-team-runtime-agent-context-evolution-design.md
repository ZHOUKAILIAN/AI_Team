# AI_Team Runtime Agent Context Evolution Design

日期：2026-04-25

## 摘要

这份方案设计 AI_Team 下一阶段的 runtime-first 架构升级。核心结论是：

> AI_Team 的阶段、状态、评判标准、证据门禁、人工卡点继续由自研 runtime 控制；OpenAI Agents SDK 作为默认 agent 执行层，负责阶段内执行、工具调用和受控 specialist/subagent 协作。
...
| 错误边界和工程化如何做 | 借鉴 Claude Code 的 hook lifecycle、permission pipeline、compaction boundary，建立 AI_Team 自己的 `RuntimeIssue` 和 fail-closed 边界 |
| 如何加入自我进化能力 | 将现有 feedback learning 从“直接写 overlay”升级为 `LearningProposal -> sandbox validation -> human review -> apply` |

## 背景

AI_Team 当前已经具备比较好的 workflow 控制基础：

| 现有模块 | 当前职责 |
| --- | --- |
| `ai_company/stage_machine.py` | 控制 Product、Dev、QA、Acceptance 之间的状态流转 |
| `ai_company/stage_policies.py` | 定义每个 stage 的目标、必需产物、证据要求、failback target |
| `ai_company/stage_contracts.py` | 将 stage policy 编译为 agent 可执行的 `StageContract` |
| `ai_company/gate_evaluator.py` | 先跑 deterministic hard gate，再按需调用独立 judge |
| `ai_company/judge_context.py` | 生成给 judge 使用的压缩上下文 `JudgeContextCompact` |
| `ai_company/openai_sandbox_judge.py` | 通过 OpenAI Agents SDK 在 sandbox 中运行独立裁判 |
| `ai_company/state.py` | 保存 session、stage run、feedback、learning overlay |
| `ai_company/roles.py` | 合并角色基础 context/memory/skill 和 learned overlay |
...
1. 不把 AI_Team 改成完全由 OpenAI Agents SDK 或 LangGraph 托管的业务流程。
...
| Claude Code 能力 | 对 AI_Team 的借鉴方式 |
| --- | --- |
| `ContextProvider` / prompt pipeline | AI_Team 增加 `ContextAssembler`，按 stage 组装稳定上下文 |
...
建议新增文件：`ai_company/execution_context.py`
...
```text
.ai-team/<session_id>/execution_context/
...
建议新增文件：`ai_company/agents/executor.py`
...
建议新增文件：`ai_company/subagents.py`
...
participant "AI_Team Runtime" as Runtime
...
建议新增文件：`ai_company/runtime_errors.py`
...
```text
ai-team verify-stage-result --judge openai-sandbox
ai-team judge-stage-result --judge openai-sandbox
```
...
```text
.ai-team/memory/<Role>/lessons.md
.ai-team/memory/<Role>/context_patch.md
.ai-team/memory/<Role>/skill_patch.md
```
...
| id | 场景 | 操作 | 期望结果 | 必需证据 |
| --- | --- | --- | --- | --- |
| AC-001 | 用户触发 AI_Team 流程 | 执行 start-session | 创建 session 并进入 Product | session.json、workflow_summary.md |
