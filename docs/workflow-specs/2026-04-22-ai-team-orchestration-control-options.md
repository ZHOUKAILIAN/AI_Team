# AI_Team Orchestration Control Options
...
如果目标是把 AI_Team 变成一个稳定、可追责、可恢复的工程系统，我的建议顺序是：
...
| 自研状态机 / runtime | 对 AI_Team 这类固定阶段流程，通常是最稳的主控方案。 |

我的偏向：

> AI_Team 最适合“自研状态机作为 control plane，OpenAI Agents SDK 作为默认 agent execution layer”。

也就是说：状态、阶段、门禁、审批、返工流转由你自己的 runtime 控制；AI 的执行、工具调用、handoff、guardrails、tracing 交给 OpenAI Agents SDK。

## 对比表

| 方案 | 核心定位 | 对固定阶段流程的控制力 | 人工审批 / 暂停恢复 | 多 agent / specialist | 工具与观测 | 代价 | 对 AI_Team 的适配判断 |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
| 自研状态机 / runtime | 你自己定义 stage/state/contract/evidence | 最强。每个 gate 都可精确编码 | 最强，前提是你自己实现 | 中。需要你自己设计 worker/handoff 机制 | 观测也要自己做 | 前期工程量最大 | 最适合 AI_Team 的主控层 |
...
但 AI_Team 的“Product 之后必须等人批准 PRD 和验收标准、QA findings 必须回 Dev、Acceptance 只能按已批准标准裁决”这种业务规则，最好还是你自己硬编码。
...
1. 保留 AI_Team 现在的 stage machine 作为唯一流程真相
...
你前面那句“评判标准这些都需要我来确定”是对的。对 AI_Team 来说，真正要被代码掌握的不是 prompt，而是每个 stage 的裁判标准。
  ...
  "artifact_index": [
    {
      ...
      "path": ".ai-team/.../acceptance_report.md",
      "sha256": "...",
...
```bash
.venv/bin/ai-team --help
```

只读试用流程：

```bash
.venv/bin/ai-team --state-root /tmp/ai-team-demo start-session \
  --message "执行这个需求：做一个支持 sandbox judge 的验收流转"

.venv/bin/ai-team --state-root /tmp/ai-team-demo acquire-stage-run \
  --session-id <session_id>

.venv/bin/ai-team --state-root /tmp/ai-team-demo submit-stage-result \
  --session-id <session_id> \
  --bundle <stage_result_bundle.json>

.venv/bin/ai-team --state-root /tmp/ai-team-demo judge-stage-result \
  --session-id <session_id> \
  --run-id <run_id> \
  --judge noop \
  --print-context
...
```bash
.venv/bin/ai-team --state-root /tmp/ai-team-demo verify-stage-result \
  --session-id <session_id> \
  --run-id <run_id> \
  --judge noop
```
...
```bash
OPENAI_API_KEY=... .venv/bin/ai-team --state-root /tmp/ai-team-demo judge-stage-result \
  --session-id <session_id> \
  --run-id <run_id> \
...
```bash
OPENAI_API_KEY=... .venv/bin/ai-team --state-root /tmp/ai-team-demo verify-stage-result \
  --session-id <session_id> \
  --run-id <run_id> \
  --judge openai-sandbox \
  --model gpt-5.4 \
...
注意：Agents SDK 的沙箱能力不等于 workflow 安全。AI_Team 仍然需要自己控制：
...
> 对 AI_Team 这种系统，真正该被代码控制的是 workflow；真正该交给模型的，是每个 stage 里的语义工作。
