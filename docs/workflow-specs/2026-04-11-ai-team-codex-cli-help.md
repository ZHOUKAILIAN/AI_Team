...
```bash
ai-team start-session --message "执行这个需求：<需求内容>"
```
...
```bash
ai-team build-stage-contract --session-id <session_id> --stage <stage_name>
```
...
```bash
ai-team record-human-decision --session-id <session_id> --decision rework --target-stage Dev
```
